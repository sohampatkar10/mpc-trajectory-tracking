...
int main(int argc, char** argv) {
  ...
  double gx = 0.5; double gy = 0.0; double gz = 0.5;
  ...
  for (i = 0; i < N; ++i) {
    acadoVariables.y[i*NY] = gx/((double) N)*i;
    acadoVariables.y[i*NY + 1] = gy/((double) N)*i;
    acadoVariables.y[i*NY + 2] = gz/((double) N)*i;
   ...
   acadoVariables.yN[0] = gx + 0.42 + 0.175;
